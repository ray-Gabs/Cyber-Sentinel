...
Cyber Sentinel combines an automated penetration testing pipeline with a real-time Security Operations Center powered by Wazuh SIEM and multi-provider AI analysis (Claude · Groq · Gemini · OpenAI).

> Once deployed, the UI is at `http://YOUR_SERVER_IP:8888` and Swagger docs at `http://YOUR_SERVER_IP:8000/docs`.
...
- Smart cross-tool deduplication — merges findings by CVE ID, template ID, or OWASP category+URL, absorbing evidence from all contributing tools
- Corroborating tool tracking — structured record of every scanner that independently confirmed the same vulnerability
- Confidence scoring — 0.0–1.0 score derived from evidence richness, cross-tool corroboration, and analyst feedback
- Analyst verdict workflow — `confirmed` / `false_positive` / `needs_investigation` with notes, auto-learned FP suppression across future scans
- Enhanced tech-stack detection — multi-source fingerprinting (WhatWeb, Nuclei template IDs, Nmap service banners, crawler response headers) for context-aware finding triage
...
- Analytics dashboard — verdict distribution, action breakdown, severity trends, triage coverage, top triggered rules
...
Access the app at **`http://YOUR_SERVER_IP:8888`** — replace `YOUR_SERVER_IP` with your server's IP or hostname.
...
<integration>
  ...
  <hook_url>http://<cyber-sentinel-host>:8000/api/alerts/webhook</hook_url>
  <level>3</level>
...
Interactive docs: `http://YOUR_SERVER_IP:8000/docs` (replace with your server address)
...
| POST | `/api/scans/{id}/verify` | Queue scan-level finding verification |
...
| PATCH | `/api/scans/{id}/findings/{idx}/verdict` | Set analyst verdict on a finding |
...
| Version | Changes |
|---|---|
| **v1.1.0** | Smart cross-tool deduplication with evidence merging · Analyst verdict workflow (confirmed / false_positive / needs_investigation) with auto-learned FP suppression · Confidence scoring (0.0–1.0) from evidence richness and cross-tool corroboration · Enhanced tech-stack detection (WhatWeb + Nuclei template IDs + Nmap banners + crawler headers) · Context-aware finding triage · Analytics dashboard fixes (verdict donut, triage coverage bar) · Security hardening and missing API endpoints — password change, bulk alert override, alert delete/history, cascade user delete, audit stats · Server-side pagination (scans + scheduled scans + alerts) · nginx security headers · SOC dedup lock fix · Scheduled scan race condition fix |
