...
.sev-strip > span { height: 100%; min-width: 2px; }
...
/* ── AI sections ── */
.ai-section { border: 1px solid var(--rule); border-radius: 10px; padding: 16px 20px; margin-bottom: 12px; background: white; }
.ai-head { display: flex; align-items: center; gap: 10px; margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px dashed var(--rule); page-break-after: avoid; break-after: avoid; }
...
function buildHtml(data: ScanReportData): string {
  ...
  const sevStripHtml = (["critical", "high", "medium", "low", "info"] as const).map(sev => {
    ...
    return cnt > 0
      ? `<span style="background:${SEV_COLOR[sev]};flex:${cnt}"></span>`
      : "";
  }).join("");
