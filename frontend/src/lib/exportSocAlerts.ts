import type { AlertSummary, AlertStats } from "@/types/alert";
import { downloadPDF } from "@/lib/pdfDownload";

export interface FilterContext {
  searchQuery?: string;
  severities?: string[];
  status?: string;
  dateFrom?: string;
  dateTo?: string;
  filterDays?: number;
  agentName?: string;
  verdict?: string;
}

export interface SocExportData {
  alerts: AlertSummary[];
  stats: AlertStats | null;
  filterContext?: FilterContext;
}

/* ── Helpers ─────────────────────────────────────────────────────────────── */

function esc(s: string | number | undefined | null): string {
  return String(s ?? "—")
    .replace(/&/g, "&amp;").replace(/</g, "&lt;")
    .replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}

function getSevLabel(level: number): string {
  if (level >= 12) return "critical";
  if (level >= 9)  return "high";
  if (level >= 6)  return "medium";
  if (level >= 3)  return "low";
  return "info";
}

const SEV_COLOR: Record<string, string> = {
  critical: "#dc2626", high: "#ea580c", medium: "#ca8a04", low: "#16a34a", info: "#475569",
};
const SEV_BG: Record<string, string> = {
  critical: "#fef2f2", high: "#fff7ed", medium: "#fefce8", low: "#f0fdf4", info: "#f1f5f9",
};

const VERDICT_STYLE: Record<string, { bg: string; color: string; label: string }> = {
  true_positive:  { bg: "#fef2f2", color: "#dc2626", label: "TRUE +" },
  false_positive: { bg: "#fefce8", color: "#a16207", label: "FALSE +" },
  unknown:        { bg: "#eff6ff", color: "#2563eb", label: "UNKNOWN" },
  unanalyzed:     { bg: "#f1f5f9", color: "#475569", label: "UNANALYZED" },
  triage_failed:  { bg: "#fff7ed", color: "#c2410c", label: "FAILED" },
};

function sevPill(label: string): string {
  const k = label.toLowerCase();
  const c = SEV_COLOR[k] ?? "#475569";
  const bg = SEV_BG[k] ?? "#f1f5f9";
  return `<span class="pill" style="--c:${c};--bg:${bg}">${esc(label.toUpperCase())}</span>`;
}

function verdictTag(raw: string): string {
  const k = raw.toLowerCase().replace(/ /g, "_");
  const s = VERDICT_STYLE[k] ?? VERDICT_STYLE.unknown;
  return `<span class="verdict" style="--c:${s.color};--bg:${s.bg}">${esc(s.label)}</span>`;
}

function barRows(rows: { label: string; count: number }[], max: number, color: string): string {
  return rows.map(({ label, count }) => {
    const pct = max > 0 ? (count / max) * 100 : 0;
    return `<div class="bar-row">
      <span class="bar-label">${esc(label)}</span>
      <div class="bar-track"><div class="bar-fill" style="width:${pct.toFixed(1)}%;background:${color}"></div></div>
      <span class="bar-count">${count}</span>
    </div>`;
  }).join("");
}

function formatTs(ts: string): { date: string; time: string } {
  if (!ts) return { date: "—", time: "" };
  const d = new Date(ts);
  return {
    date: d.toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" }),
    time: d.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", second: "2-digit" }),
  };
}

/* ── HTML builder ────────────────────────────────────────────────────────── */

function buildFilterDescription(fc?: FilterContext): string {
  if (!fc) return "";
  const parts: string[] = [];
  if (fc.searchQuery) parts.push(`Search: "${esc(fc.searchQuery)}"`);
  if (fc.severities?.length) parts.push(`Severity: ${fc.severities.map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(", ")}`);
  if (fc.status && fc.status !== "all") parts.push(`Status: ${esc(fc.status)}`);
  if (fc.verdict) parts.push(`Verdict: ${esc(fc.verdict.replace(/_/g, " "))}`);
  if (fc.agentName) parts.push(`Agent: ${esc(fc.agentName)}`);
  if (fc.dateFrom || fc.dateTo) {
    parts.push(`Date: ${esc(fc.dateFrom || "any")} – ${esc(fc.dateTo || "now")}`);
  } else if (fc.filterDays) {
    parts.push(`Range: Last ${fc.filterDays}d`);
  }
  return parts.length ? parts.join(" · ") : "No filters active";
}

function buildHtml(data: SocExportData): string {
  const { alerts, stats, filterContext } = data;
  const now = new Date();
  const generated = now.toLocaleString("en-US", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" });
  const dateStr = now.toISOString().slice(0, 10);

  const bySev = stats?.by_severity ?? {};
  const total = stats?.total ?? alerts.length;

  const sevCounts = {
    critical: bySev["critical"] ?? 0,
    high:     bySev["high"]     ?? 0,
    medium:   bySev["medium"]   ?? 0,
    low:      bySev["low"]      ?? 0,
    info:     bySev["info"]     ?? 0,
  };

  const kpiCards = [
    { label: "Total Alerts", value: total,              color: "#2563eb" },
    { label: "Critical",     value: sevCounts.critical, color: "#dc2626" },
    { label: "High",         value: sevCounts.high,     color: "#ea580c" },
    { label: "Medium",       value: sevCounts.medium,   color: "#ca8a04" },
    { label: "Low",          value: sevCounts.low,      color: "#16a34a" },
    { label: "Info",         value: sevCounts.info,     color: "#475569" },
  ].map(({ label, value, color }) =>
    `<div class="kpi-card" style="--bar:${color}">
      <span class="kpi-label">${esc(label)}</span>
      <span class="kpi-value" style="color:${color}">${value}</span>
    </div>`
  ).join("");

  const topAgents = (stats?.top_agents ?? []).slice(0, 8).map(x => ({ label: x._id, count: x.count }));
  const topRules  = (stats?.top_rules  ?? []).slice(0, 8).map(x => ({ label: `${x._id} — ${x.desc}`.slice(0, 50), count: x.count }));
  const maxAgent = Math.max(...topAgents.map(x => x.count), 1);
  const maxRule  = Math.max(...topRules.map(x => x.count), 1);

  // Severity distribution strip
  const sevTotal = sevCounts.critical + sevCounts.high + sevCounts.medium + sevCounts.low + sevCounts.info || 1;
  const sevStrip = [
    { sev: "critical", label: "Critical", color: "#dc2626" },
    { sev: "high",     label: "High",     color: "#ea580c" },
    { sev: "medium",   label: "Medium",   color: "#ca8a04" },
    { sev: "low",      label: "Low",      color: "#16a34a" },
    { sev: "info",     label: "Info",     color: "#475569" },
  ].map(({ sev, label, color }) => {
    const pct = (sevCounts[sev as keyof typeof sevCounts] / sevTotal * 100).toFixed(1);
    return `<div class="sev-seg" style="width:${pct}%;background:${color}" title="${label}: ${sevCounts[sev as keyof typeof sevCounts]}"></div>`;
  }).join("");

  const tableRows = alerts.map((a, i) => {
    const { date, time } = formatTs(a.timestamp ?? "");
    const sevLabel = getSevLabel(a.rule_level ?? 0);
    const verdict  = a.ai_verdict ?? "unanalyzed";
    return `<tr>
      <td class="c-ix">${i + 1}</td>
      <td class="c-time"><span class="ts-date">${esc(date)}</span><span class="ts-time">${esc(time)}</span></td>
      <td class="c-agent">${esc(a.agent_name ?? "—")}</td>
      <td class="c-sev">${sevPill(sevLabel)}</td>
      <td class="c-desc">${esc(a.rule_description ?? "—")}</td>
      <td class="c-rule">${esc(a.rule_id ?? "—")}</td>
      <td class="c-verd">${verdictTag(verdict)}</td>
    </tr>`;
  }).join("");

  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>SOC Alert Feed — Cyber Sentinel</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">
<style>
@page { size: A4 landscape; margin: 0; }
@media print {
  body { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  .no-print { display: none !important; }
  thead { display: table-header-group; }
  tr { page-break-inside: avoid; }
}
*,*::before,*::after { box-sizing: border-box; margin: 0; padding: 0; }
:root {
  --bg: #fff; --paper: #fafaf7; --ink: #18181b; --ink2: #52525b; --ink3: #a1a1aa;
  --border: #e4e4e7; --border2: #f4f4f5; --accent: #2563eb;
}
body { background: var(--bg); color: var(--ink); font-family: 'Inter', system-ui, sans-serif; font-size: 10px; line-height: 1.5; }
.page { padding: 12mm 14mm 18mm; position: relative; }
.page + .page { page-break-before: always; }
/* Header */
.page-header { display: flex; align-items: flex-start; justify-content: space-between; padding-bottom: 10px; border-bottom: 2px solid var(--accent); margin-bottom: 14px; }
.brand { display: flex; align-items: center; gap: 8px; margin-bottom: 6px; }
.mark { width: 20px; height: 20px; border-radius: 4px; background: linear-gradient(135deg, #2563eb, #1e3a8a); position: relative; flex-shrink: 0; }
.mark::after { content: ''; position: absolute; inset: 5px; background: white; clip-path: polygon(50% 0, 100% 25%, 100% 65%, 50% 100%, 0 65%, 0 25%); }
.brand-name { font-size: 10.5px; font-weight: 700; color: var(--ink); }
.brand-sub  { font-size: 8px; color: var(--ink3); letter-spacing: 0.08em; text-transform: uppercase; }
.doc-eyebrow { font-size: 8px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: var(--accent); margin-bottom: 4px; }
.doc-title { font-size: 18px; font-weight: 800; letter-spacing: -0.025em; line-height: 1.2; }
.doc-sub { font-size: 9.5px; color: var(--ink2); margin-top: 3px; }
.meta { text-align: right; font-size: 8px; color: var(--ink3); line-height: 1.9; }
.conf-pill { display: inline-block; border: 1px solid var(--border); color: var(--ink2); font-size: 7px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; padding: 2px 7px; border-radius: 3px; margin-bottom: 3px; }
/* KPI */
.kpi-strip { display: flex; gap: 10px; margin-bottom: 12px; }
.kpi-card { flex: 1; border: 1px solid var(--border); border-radius: 6px; padding: 9px 11px 9px 14px; position: relative; overflow: hidden; background: var(--paper); }
.kpi-card::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 3px; background: var(--bar, var(--accent)); border-radius: 6px 0 0 6px; }
.kpi-label { display: block; font-size: 7.5px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: var(--ink3); margin-bottom: 3px; }
.kpi-value { display: block; font-size: 22px; font-weight: 700; font-family: 'JetBrains Mono', monospace; letter-spacing: -0.04em; }
/* Sev strip */
.sev-strip { display: flex; height: 7px; border-radius: 4px; overflow: hidden; margin-bottom: 14px; }
.sev-seg { height: 100%; min-width: 2px; }
/* Charts */
.charts { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 14px; }
.chart-box { border: 1px solid var(--border); border-radius: 6px; padding: 11px 13px; background: var(--paper); }
.chart-title { font-size: 7.5px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: var(--ink3); margin-bottom: 9px; }
.bar-row { display: grid; grid-template-columns: 130px 1fr 36px; align-items: center; gap: 8px; margin-bottom: 5px; }
.bar-label { font-size: 8.5px; color: var(--ink2); font-family: 'JetBrains Mono', monospace; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.bar-track { height: 5px; background: var(--border2); border-radius: 3px; overflow: hidden; }
.bar-fill { height: 100%; border-radius: 3px; }
.bar-count { font-size: 8.5px; font-family: 'JetBrains Mono', monospace; color: var(--ink3); text-align: right; }
/* Table */
.section-label { font-size: 7.5px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: var(--ink3); margin-bottom: 7px; }
table.alerts { width: 100%; border-collapse: collapse; }
table.alerts thead tr { background: var(--border2); }
table.alerts th { font-size: 7.5px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--ink2); padding: 6px 10px; text-align: left; border-bottom: 2px solid var(--border); white-space: nowrap; }
table.alerts td { padding: 5px 10px; border-bottom: 1px solid var(--border2); vertical-align: middle; font-size: 9.5px; color: var(--ink2); }
td.c-ix { width: 28px; color: var(--ink3); font-family: 'JetBrains Mono', monospace; font-size: 8.5px; text-align: center; }
td.c-time { width: 115px; }
.ts-date { display: block; font-size: 9px; color: var(--ink); }
.ts-time { display: block; font-size: 8px; color: var(--ink3); font-family: 'JetBrains Mono', monospace; }
td.c-agent { width: 110px; font-family: 'JetBrains Mono', monospace; font-size: 8.5px; }
td.c-sev { width: 80px; }
td.c-desc { }
td.c-rule { width: 70px; font-family: 'JetBrains Mono', monospace; font-size: 8.5px; color: var(--ink3); }
td.c-verd { width: 100px; }
/* Badges */
.pill { display: inline-flex; align-items: center; gap: 4px; padding: 2px 7px; border-radius: 999px; font-size: 7.5px; font-weight: 700; letter-spacing: 0.06em; text-transform: uppercase; background: var(--bg); color: var(--c, #475569); border: 1px solid var(--c, #475569); }
.pill::before { content: ''; width: 5px; height: 5px; border-radius: 50%; background: var(--c, #475569); flex-shrink: 0; }
.verdict { display: inline-block; padding: 2px 7px; border-radius: 4px; font-size: 7.5px; font-weight: 700; letter-spacing: 0.06em; text-transform: uppercase; background: var(--bg, #f1f5f9); color: var(--c, #475569); border: 1px solid var(--c, #475569); }
/* Footer */
.page-footer { margin-top: 14px; padding-top: 6px; border-top: 1px solid var(--border); display: flex; justify-content: space-between; font-size: 7.5px; color: var(--ink3); }
/* Print button */
.print-btn { position: fixed; bottom: 20px; right: 20px; background: #2563eb; color: white; border: none; padding: 10px 20px; border-radius: 6px; font-size: 12px; font-weight: 600; cursor: pointer; z-index: 9999; font-family: inherit; box-shadow: 0 4px 12px rgba(37,99,235,.3); }
.print-btn:hover { background: #1d4ed8; }
</style>
</head>
<body>
<button class="print-btn no-print" onclick="window.print()">Print / Save PDF</button>
<div class="page">
  <div class="page-header">
    <div>
      <div class="brand">
        <div class="mark"></div>
        <div>
          <div class="brand-name">Cyber Sentinel</div>
          <div class="brand-sub">Security Platform</div>
        </div>
      </div>
      <div class="doc-eyebrow">Smart City and Cybersecurity Lab &middot; ITS</div>
      <div class="doc-title">SOC Alert Feed</div>
      <div class="doc-sub">Wazuh alert export &middot; ${alerts.length} records &middot; ${esc(generated)}</div>
      ${filterContext ? `<div style="margin-top:5px;font-size:8px;color:#52525b;padding:4px 8px;background:#f4f4f5;border-radius:4px;border:1px solid #e4e4e7;display:inline-block">Active filters: ${buildFilterDescription(filterContext)}</div>` : ""}
    </div>
    <div class="meta">
      <div class="conf-pill">Confidential</div>
      <div>Generated ${esc(generated)}</div>
      <div>${total.toLocaleString()} total alerts</div>
      <div>Cyber Sentinel v1.0</div>
    </div>
  </div>
  <div class="kpi-strip">${kpiCards}</div>
  <div class="sev-strip">${sevStrip}</div>
  <div class="charts">
    <div class="chart-box">
      <div class="chart-title">Top Agents</div>
      ${topAgents.length ? barRows(topAgents, maxAgent, "#2563eb") : '<span style="font-size:9px;color:var(--ink3)">No data</span>'}
    </div>
    <div class="chart-box">
      <div class="chart-title">Top Rules</div>
      ${topRules.length ? barRows(topRules, maxRule, "#dc2626") : '<span style="font-size:9px;color:var(--ink3)">No data</span>'}
    </div>
  </div>
  <div class="section-label">Alert Feed &mdash; ${alerts.length} Records</div>
  <table class="alerts">
    <thead>
      <tr>
        <th style="width:28px">#</th>
        <th style="width:115px">Time</th>
        <th style="width:110px">Agent</th>
        <th style="width:80px">Severity</th>
        <th>Description</th>
        <th style="width:70px">Rule ID</th>
        <th style="width:100px">Verdict</th>
      </tr>
    </thead>
    <tbody>${tableRows}</tbody>
  </table>
  <div class="page-footer">
    <span>Cyber Sentinel &mdash; Smart City and Cybersecurity Lab &middot; ITS</span>
    <span>${esc(dateStr)}</span>
  </div>
</div>
</body>
</html>`;
}

/* ── Entry point ─────────────────────────────────────────────────────────── */

export async function exportSocAlertsPDF(data: SocExportData): Promise<void> {
  const html    = buildHtml(data);
  const dateStr = new Date().toISOString().slice(0, 10);
  await downloadPDF(html, `soc-alerts-${dateStr}`);
}
